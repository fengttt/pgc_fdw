...
/*
 * Valid options for pgc_fdw.
 * Allocated and filled in InitPgFdwOptions.
 */
static PgFdwOption *pgc_fdw_options;
 ...
 * USER MAPPING or FOREIGN TABLE that uses pgc_fdw.
 *
 * Raise an ERROR if the option or its value is considered invalid.
 */
PG_FUNCTION_INFO_V1(pgc_fdw_validator);

Datum
pgc_fdw_validator(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Check that only options supported by pgc_fdw, and allowed for the
	...
	foreach(cell, options_list)
	{
		...
		if (!is_valid_option(def->defname, catalog))
		{
			...
			initStringInfo(&buf);
			for (opt = pgc_fdw_options; opt->keyword; opt++)
			{
...
InitPgFdwOptions(void)
{
	 ...
	 * pgc_fdw, by appending FDW-specific options to libpq options.
	 *
	 * We use plain malloc here to allocate pgc_fdw_options because it
	 ...
	 * string.
	 */
	pgc_fdw_options = (PgFdwOption *)
		malloc(sizeof(PgFdwOption) * num_libpq_opts +
			   sizeof(non_libpq_options));
	if (pgc_fdw_options == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_FDW_OUT_OF_MEMORY),
				 errmsg("out of memory")));

	popt = pgc_fdw_options;
	for (lopt = libpq_options; lopt->keyword; lopt++)
	{
...
/*
 * Check whether the given option is one of the valid pgc_fdw options.
...
is_valid_option(const char *keyword, Oid context)
{
	...
	Assert(pgc_fdw_options);	/* must be initialized already */
