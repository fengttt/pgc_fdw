...
CREATE SERVER testserver1 FOREIGN DATA WRAPPER pgc_fdw;
DO $d$
    BEGIN
        EXECUTE $$CREATE SERVER loopback FOREIGN DATA WRAPPER pgc_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
        EXECUTE $$CREATE SERVER loopback2 FOREIGN DATA WRAPPER pgc_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
    END;
...
CREATE FUNCTION pgc_fdw_abs(int) RETURNS int AS $$
BEGIN
RETURN abs($1);
END
...
EXPLAIN (VERBOSE, COSTS OFF)
  SELECT count(c3) FROM ft1 t1 WHERE t1.c1 = pgc_fdw_abs(t1.c2);
SELECT count(c3) FROM ft1 t1 WHERE t1.c1 = pgc_fdw_abs(t1.c2);
EXPLAIN (VERBOSE, COSTS OFF)
  SELECT count(c3) FROM ft1 t1 WHERE t1.c1 === t1.c2;
SELECT count(c3) FROM ft1 t1 WHERE t1.c1 === t1.c2;
...
ALTER EXTENSION pgc_fdw ADD FUNCTION pgc_fdw_abs(int);
ALTER EXTENSION pgc_fdw ADD OPERATOR === (int, int);
ALTER SERVER loopback OPTIONS (ADD extensions 'pgc_fdw');
...
EXPLAIN (VERBOSE, COSTS OFF)
SELECT t1.c1, t2.c2, t1.c3 FROM ft1 t1 FULL JOIN ft2 t2 ON (t1.c1 = t2.c1) WHERE pgc_fdw_abs(t1.c1) > 0 OFFSET 10 LIMIT 10;
...
alter extension pgc_fdw add function least_accum(anyelement, variadic anyarray);
alter extension pgc_fdw add aggregate least_agg(variadic items anyarray);
alter server loopback options (set extensions 'pgc_fdw');
...
alter extension pgc_fdw drop function least_accum(anyelement, variadic anyarray);
alter extension pgc_fdw drop aggregate least_agg(variadic items anyarray);
alter server loopback options (set extensions 'pgc_fdw');
...
-- Add into extension
alter extension pgc_fdw add operator class my_op_class using btree;
alter extension pgc_fdw add function my_op_cmp(a int, b int);
alter extension pgc_fdw add operator family my_op_family using btree;
alter extension pgc_fdw add operator public.<^(int, int);
alter extension pgc_fdw add operator public.=^(int, int);
alter extension pgc_fdw add operator public.>^(int, int);
alter server loopback options (set extensions 'pgc_fdw');
...
alter extension pgc_fdw drop operator class my_op_class using btree;
alter extension pgc_fdw drop function my_op_cmp(a int, b int);
alter extension pgc_fdw drop operator family my_op_family using btree;
alter extension pgc_fdw drop operator public.<^(int, int);
alter extension pgc_fdw drop operator public.=^(int, int);
alter extension pgc_fdw drop operator public.>^(int, int);
alter server loopback options (set extensions 'pgc_fdw');
...
EXPLAIN (verbose, costs off)
UPDATE ft2 SET c3 = 'bar' WHERE pgc_fdw_abs(c1) > 2000 RETURNING *;            -- can't be pushed down
UPDATE ft2 SET c3 = 'bar' WHERE pgc_fdw_abs(c1) > 2000 RETURNING *;
EXPLAIN (verbose, costs off)
...
CREATE SERVER fetch101 FOREIGN DATA WRAPPER pgc_fdw OPTIONS( fetch_size '101' );
...
GRANT USAGE ON FOREIGN DATA WRAPPER pgc_fdw TO regress_nosuper;
...
DO $d$
    BEGIN
        EXECUTE $$CREATE SERVER loopback_nopw FOREIGN DATA WRAPPER pgc_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
    END;
