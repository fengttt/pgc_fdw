 ...
 *
 * connection.c
 *		  Connection management functions for pgc_fdw
 *
 * Portions Copyright (c) 2012-2020, PostgreSQL Global Development Group
 *
 * IDENTIFICATION
 *		  contrib/pgc_fdw/connection.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
GetConnection(UserMapping *user, bool will_prep_stmt)
{
	...
	if (ConnectionHash == NULL)
	{
		...
		ConnectionHash = hash_create("pgc_fdw connections", 8,
									 &ctl,
	...
	if (entry->conn == NULL)
	{
		...
		elog(DEBUG3, "new pgc_fdw connection %p for server \"%s\" (user mapping oid %u, userid %u)",
...
connect_pg_server(ForeignServer *server, UserMapping *user)
{
	...
	PG_TRY();
	{
		...
		/* Use "pgc_fdw" as fallback_application_name. */
		...
		if (entry->xact_depth > 0)
		{
			...
			switch (event)
			{
				...
				case XACT_EVENT_PRE_COMMIT:
					 ...
					 * constrains how old a server pgc_fdw can
				...
				case XACT_EVENT_PRE_PREPARE:
					...
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							 errmsg("cannot PREPARE a transaction that has operated on pgc_fdw foreign tables")));
					break;
